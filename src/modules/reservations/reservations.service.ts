...
import { Reservation, ReservationStatus } from './entities/reservation.entity.js';
...
export class ReservationsService {
  ...
  async update(id: string, updateReservationDto: UpdateReservationDto) {
    ...
    const customer_id = updateReservationDto.customer_id ?? reservation.customer_id;
  ...
  private async ensureCustomerExists(customer_id: string) {
    ...
    if (!customer) {
      throw new NotFoundException(`Cliente con id ${customer_id} no encontrado`);
    }
  }
  ...
  private ensureNotInPast(date: string, time: string) {
    ...
    if (!Number.isNaN(when.getTime()) && when.getTime() < Date.now()) {
      throw new BadRequestException('No se puede reservar en una fecha u hora pasada');
    }
  }
}